...
class FieldsCorrect(object):
    def __init__(self, sinogram_size:torch.Size, flat_field=None, dark_field=None) -> None:
    ...
    def __call__(self, input_dict:Dict) -> Dict:
        input_dict.update({'sinogram':(input_dict['sinogram']-self.dark_field)/(self.flat_field-self.dark_field)})
        return input_dict
...
class FieldsUncorrect(object):
    ...
    def __call__(self, input_dict:Dict) -> Dict:
        input_dict.update({'sinogram':input_dict['sinogram']*(self.flat_field-self.dark_field)+self.dark_field})
        return input_dict
...
class Exponentiate(object):
    ...
    def __call__(self, input_dict:Dict)-> Dict:
        input_dict.update({'sinogram':self.IO*torch.exp(-input_dict['sinogram']/input_dict['max_value'])})
        return input_dict
...
class Logarithm(object):
    ...
    def __call__(self, input_dict:Dict)-> torch.Tensor:
        sinogram =-torch.log(input_dict['sinogram']/self.IO)*input_dict['max_value']
...
class DetectorCrossTalk(object):
    def __init__(self, cross_talk:float):
        kernel = torch.tensor([[0., 0., 0.],
                                    ...
                                    [0., 0., 0.]]).view(1, 1, 3, 3).repeat(1, 1, 1, 1)/(1+2*cross_talk)
...
class ElectronicNoise(object):
    ...
    def __call__(self, input_dict:Dict) -> Dict:
        noisy_tensor = input_dict['sinogram'] + self.sigma * torch.zeros(size = input_dict['sinogram'].size()).normal_()
        noisy_tensor[noisy_tensor<=0]=1e-6
        input_dict.update({'sinogram':noisy_tensor})
        return input_dict

class CompleteSinogramTransform(object):
    def __init__(self, I0:int, sinogram_size:torch.Size, sigma:int, cross_talk:float, flat_field=None, dark_field=None):
