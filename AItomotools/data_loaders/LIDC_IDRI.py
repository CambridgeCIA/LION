...
def get_slice(scan: pl.Scan, slice_index: int) -> np.ndarray:
    ...
    image = dicom.read_file(
        pathlib.Path(path_to_dicom_files).joinpath(f"{slice_index}.dcm")
    )
    input()
    return (image.pixel_array * image.RescaleSlope + image.RescaleIntercept).astype(
        np.int16
    )
...
def is_nodule_on_slice(
    annotation: pl.Annotation, slice_index: int, verbose=False
) -> Tuple[bool, int]:
    ...
    if zmin <= slice_index <= zmax:
        if verbose:
            print(
                f"The annotated nodule {annotation.id} is present on slice {slice_index}"
            )
        return True, zmin
    ...
    for annotated_nodule in list_of_annotated_nodules:
        ...
        if tumor_on_slice:
            ...
            nodule_array[xmin:xmax, ymin:ymax] = np.bitwise_or(
                nodule_array[xmin:xmax, ymin:ymax], mask
            )
            background_array[xmin:xmax, ymin:ymax] = np.bitwise_and(
                background_array[xmin:xmax, ymin:ymax], np.invert(mask)
            )

    return (
        np.dstack((background_array, nodule_array))
        .transpose((2, 1, 0))
        .transpose((0, 2, 1))
    )
    ...
    def __init__(
        ...
        if self.mode == "training":
            self.slice_index_to_patient_id_list = (
                self.get_slice_index_to_patient_id_list(self.training_patients_list)
            )
            self.patient_id_to_first_index_dict = (
                self.get_patient_id_to_first_index_dict(self.training_patients_list)
            )

        elif self.mode == "testing":
            self.slice_index_to_patient_id_list = (
                self.get_slice_index_to_patient_id_list(self.testing_patients_list)
            )
            self.patient_id_to_first_index_dict = (
                self.get_patient_id_to_first_index_dict(self.testing_patients_list)
            )
