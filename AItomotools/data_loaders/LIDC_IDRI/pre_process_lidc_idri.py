...
def process_patient_statistics(scan: pl.Scan):
    ...
    for annotated_nodule in list_of_annotated_nodules:
        for annotation in annotated_nodule:
            n_slices_patient += (
                annotation.bbox()[-1].stop - annotation.bbox()[-1].start
            )  # type:ignore
    ...
    for patient_index in range(1, 1012):
        ...
        if type(scan) == type(None):
            ...
            if len(list(path_to_processed_volume_folder.glob("*"))) == 0:
                ...
                n_annotated_nodules, n_segmented_slices = process_patient_statistics(
                    scan
                )
        ...
        with open(
            path_to_processed_dataset.joinpath("patient_id_to_n_segmented_slices.json"),
            "w",
        ) as out_f:
            json.dump(patient_id_to_n_segmented_slices, out_f)
        with open(
            path_to_processed_dataset.joinpath("patient_id_to_n_nodules.json"), "w"
        ) as out_f:
            json.dump(patient_id_to_n_nodules, out_f)
        with open(
            path_to_processed_dataset.joinpath("patients_masks.json"), "w"
        ) as out_f:
...
def compute_diagnosis_file(
    ...
    if diagnosis_dict_save_path.is_file():
        raise FileExistsError(
            f"There is already a file at {diagnosis_dict_save_path}, passing"
        )
...
if __name__ == "__main__":
    path_to_raw_dataset = pathlib.Path(
        "/local/scratch/public/AItomotools/raw/LIDC-IDRI"
    )
