...
class LunaImage(CTimage):
    ...
    def set_nodule_data(self, nodule_list):
        ...
        if not isinstance(nodule_list, list) and not all(
            isinstance(n, Nodule) for n in nodule_list
        ):
    ...
    def get_croped_nodule_slices(self, indices):
        ...
        if not isinstance(indices, list):
            if indices == "all":
                ...
                raise ValueError(
                    "Input indices need to be an index (or list of indices) of the Nodules"
                )
        ...
        for nodule in nodule_list:
            ...
            cropped.append(
                self.data[
                    nodule_center[0]
                    - nodule_radious[0] : nodule_center[0]
                    + nodule_radious[0]
                ]
            )
    ...
    def get_nodule(self, nodule):
        ...
        nodule_radious = np.ceil(nodule.diameter / self.spacing / 2).astype(int) + 1
        return self.data[
            nodule_center[0] - nodule_radious[0] : nodule_center[0] + nodule_radious[0],
    ...
    def make_nodule_only_mask(self, indices):
        ...
        for nodule in nodule_list:
            ...
            self.nodule_mask[
                ...
                + nodule_radious[0],
                nodule_center[1]
                - nodule_radious[1] : nodule_center[1]
                + nodule_radious[1],
                nodule_center[2]
                - nodule_radious[2] : nodule_center[2]
                + nodule_radious[2],
            ] = label
            label = label + 1
    ...
    def make_binary_mask_nodule(self, indices):
        ...
        for nodule in nodule_list:
            ...
            our_blob = blobs_labels[
                blobs_labels.shape[0] // 2,
                blobs_labels.shape[1] // 2,
                blobs_labels.shape[2] // 2,
            ]
...
class LUNA16(CT_data_loader):
    ...
    def load_metadata(self):
        ...
        for sub_folder_name in self.subfolders:
            ...
            self.images.extend(
                [LunaImage(curr_folder, f) for f in files]
            )  # if os.path.splitext(f)[0] in nodulefiles # This lets only load images with nodules
