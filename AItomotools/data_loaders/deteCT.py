...
class ScanDataset(Dataset):
    def __init__(
        ...
        if default:
            self.path_to_data_record = self.path_to_dataset.joinpath(f'default_data_records.csv')
        else:
            self.path_to_data_record = self.path_to_dataset.joinpath(f'complete_data_records.csv')
        ...
        assert pipeline in ["reconstruction", "segmentation", "joint"], f'Wrong pipeline argument, must be in ["reconstruction", "segmentation", "joint"]'
        if pipeline in ["segmentation", "joint"]:
            assert self.mode == 'mode2', f'Inconsistent arguments. The mode is set on {self.mode} which does not contain the segmentation that {pipeline} pipeline requires'
        ...
        """
        if self.train:
            self.sample_dataframe = self.sample_dataframe.head(int(len(self.sample_dataframe)*self.training_proportion))
        else:
            self.sample_dataframe = self.sample_dataframe.tail(int(len(self.sample_dataframe)*(1-self.training_proportion)))

        self.slice_dataframe = self.slice_dataframe[self.slice_dataframe['sample_index'].isin(self.sample_dataframe['sample_index'].unique())]
    ...
    def compute_sample_dataframe(self):
        ...
        for identifier in unique_identifiers:
            ...
            subset = self.slice_dataframe[self.slice_dataframe['sample_index'] == identifier]
    ...
    def __len__(self):
        return self.sample_dataframe['last_slice'].iloc[-1] - self.sample_dataframe['first_slice'].iloc[0] +1

    def __getitem__(self, index):
        slice_row = self.slice_dataframe.iloc[index]
        path_to_slice = self.path_to_dataset.joinpath(f"{slice_row['slice_identifier']}/{self.mode}")
        reconstruction = torch.from_numpy(np.load(path_to_slice.joinpath('reconstruction.npy'))).unsqueeze(0)
        if self.pipeline == 'segmentation':
            segmentation = torch.from_numpy(np.load(path_to_slice.joinpath('reconstruction.npy'))).unsqueeze(0)
            tensor_dict = {
                'reconstruction':reconstruction,
                'segmentation':segmentation
                }
        if self.pipeline in 'reconstruction':
            sinogram = torch.from_numpy(np.load(path_to_slice.joinpath('sinogram.npy'))).unsqueeze(0)
            tensor_dict = {
                'reconstruction':reconstruction,
                'sinogram':sinogram
                }
