...
class Luna16Dataset(Dataset):
    def __init__(self, device : torch.device, mode : str, sinogram_transform=None, image_transform=None) -> None:
        assert mode in ['testing', 'training', 'validation'], f'Mode argument {mode} not in ["testing", "training", "validation"]'
        self.device = device
        self.mode = mode
    ...
    def __getitem__(self, index):
        ...
        assert image.size() == torch.Size(self.geometry.image_shape), f'Queried image size {image.size()} != expected size from geometry {self.geometry.image_shape}'
