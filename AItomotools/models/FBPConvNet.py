...
class ConvBlock(nn.Module):
    def __init__(self,layers,channels,relu_type='ReLU',relu_last=True,kernel_size=3):
        super().__init__()
        # input parsing:
        ...
        for ii in range(layers):
            layer_list.append(nn.Conv2d(channels[ii], channels[ii + 1], kernel_size, padding=1, bias=False))
...
class Up(nn.Module):
    ...
    def __init__(self,channels,stride=2,relu_type='ReLU'):
        ...
        kernel_size=3
        layer_list=[]
        layer_list.append(nn.ConvTranspose2d(channels[0],channels[1],kernel_size,padding=1,output_padding=1,stride=stride,bias=False))
