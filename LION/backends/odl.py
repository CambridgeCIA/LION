...
class ODLBackend:
    ...
    def initialise_detector_partition_from_dict(
        self, detector_partition_dict: Dict
    ) -> None:
    ...
    def initialise_geometry_from_dict(self, geometry_dict: Dict) -> None:
        assert (
            self.angle_partition is not None
        ), "Angle partition not initialised, consider running odl_backend.initialise_angle_partition_from_dict"
        assert (
            self.detector_partition is not None
        ), "Angle partition not initialised, consider running odl_backend.initialise_detector_partition_from_dict"
        if geometry_dict["beam_geometry"] == "fan_beam":
            ...
            raise NotImplementedError(
                f"Beam geometry {geometry_dict['beam_geometry']} not implemented"
            )

    def set_operator(self) -> None:
        assert (
            self.space is not None
        ), "space attribute not initialised, consider running odl_backend.initialise_space_from_dict"
        assert (
            self.geometry is not None
        ), "space attribute not initialised, consider running odl_backend.set_geometry"
        self.operator = odl.tomo.RayTransform(self.space, self.geometry)

    def set_pytorch_operator(self):
        assert (
            self.operator is not None
        ), "operator attribute not initialised, consider running odl_backend.set_operator"
    ...
    def initialise_odl_backend_from_metadata_dict(self, metadata_dict: Dict):
        ...
        self.initialise_detector_partition_from_dict(
            metadata_dict["detector_partition_dict"]
        )
    ...
    def get_reconstruction(self, sinogram: torch.Tensor) -> torch.Tensor:
        ...
        return (
            self.pytorch_operator(sinogram) / self.pytorch_operator_norm
        )  # type:ignore
