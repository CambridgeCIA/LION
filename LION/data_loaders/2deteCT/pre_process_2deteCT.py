...
def write_settings(settings_out_file_path: Path):
    ...
    settings = {
        ...
        "angular": {"n_angles": 3600, "subsampling": 1},
        "reconstruction": {
            "n_pixels": [512, 512],
            "n_voxels": 512,
            "SOD": 431.019989,
    ...
    settings["detector"]["pixel_physical_size_corrected"] = (
        settings["detector"]["subsampling"]
        * settings["detector"]["binning"]
        * settings["detector"]["pixel_physical_size_reference"]
    )
    settings["detector"]["n_pixels_subsampled"] = (
        settings["detector"]["n_pixels"] // settings["detector"]["subsampling"]
    )
    settings["detector"]["width"] = (
        settings["detector"]["pixel_physical_size_corrected"]
        * settings["detector"]["n_pixels_subsampled"]
    )
    # Physical width of the field of view in the measurement plane via intersect theorem.
    settings["reconstruction"]["FOV_width"] = (
        settings["detector"]["width"]
        * settings["reconstruction"]["SOD"]
        / settings["reconstruction"]["SDD"]
    )

    settings["reconstruction"]["voxel_size"] = (
        settings["reconstruction"]["FOV_width"] / settings["reconstruction"]["n_voxels"]
    )

    # Scaling the geometry accordingly.
    settings["reconstruction"]["scale_factor"] = (
        1.0 / settings["reconstruction"]["voxel_size"]
    )
    settings["reconstruction"]["scaled_SDD"] = (
        settings["reconstruction"]["SDD"] * settings["reconstruction"]["scale_factor"]
    )
    settings["reconstruction"]["scaled_SOD"] = (
        settings["reconstruction"]["SOD"] * settings["reconstruction"]["scale_factor"]
    )

    settings["detector"]["scaled_pixel_physical_size"] = (
        settings["detector"]["pixel_physical_size_corrected"]
        * settings["reconstruction"]["scale_factor"]
    )
...
def pre_process_slice(
    ...
    folder_savepath = processed_folder_path.joinpath(
        f"{formatted_slice_index}/{formatted_mode}"
    )
    ...
    if mode_index == 2:
        segmentation = np.array(
            imageio.imread(path_to_mode.joinpath("segmentation.tif"))
        )
...
if __name__ == "__main__":
    ...
    settings = dict(
        json.load(open(processed_folder_path.joinpath("scan_settings.json")))
    )
