...
from LION.models.CNNs.UNets.Fixed_Conv_UNet import Fixed_Conv_UNet
class ItNet(LIONmodel.LIONmodel):
    def __init__(self, geometry: ct.Geometry, model_parameters: LIONParameter = None):
        ...
        for i in range(self.model_parameters.n_iters):
            self.add_module(f"Fixed_Conv_UNet_{i}", UNet(self.model_parameters.Fixed_Conv_UNet_params))
    ...
    def default_parameters():
        ...
        param.n_iters = 4
        param.Fixed_Conv_UNet_params = UNet.default_parameters()
        param.mode = "ct"
        return param
