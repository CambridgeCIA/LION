...
from LION.models.CNNs.UNets.Fixed_Conv_UNetScharr import Fixed_Conv_UNet


class ItNetScharr(LIONmodel.LIONmodel):
    def __init__(self, geometry: ct.Geometry, model_parameters: LIONParameter = None):
        if geometry is None:
            raise ValueError("Geometry parameters required. ")
            # geometry = ct.default_parameters()

        super().__init__(model_parameters, geometry)

        # Create layers per iteration
        for i in range(self.model_parameters.n_iters):
            self.add_module(f"Fixed_Conv_UNet_{i}", Fixed_Conv_UNet(self.model_parameters.Fixed_Conv_UNet_params))
    ...
    def default_parameters():
        ...
        param.n_iters = 4
        param.Fixed_Conv_UNet_params = Fixed_Conv_UNet.default_parameters()
        param.mode = "ct"
        return param
    ...
    def forward(self, sino):
        ...
        for i in range(self.model_parameters.n_iters):
            unet = getattr(self, f"Fixed_Conv_UNet_{i}")
            img = unet(img)
