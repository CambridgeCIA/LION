...
my_file = savefolder.joinpath("LPD_final_iter.pt")
if my_file.is_file():
    print("Final version found, no need to loop further, exiting")
    exit()
my_file = savefolder.joinpath("LPD_min_val.pt")
if my_file.is_file():
...
for epoch in range(start_epoch, train_param.epochs):
    ...
    if min_valid_loss > valid_loss:
        ...
        # Saving State Dict
        model.save(
            savefolder.joinpath(f"LPD_min_val.pt"),
            epoch=epoch + 1,
    ...
    if epoch % 10 == 0:
        model.save_checkpoint(
            savefolder.joinpath(f"LPD_check_{epoch+1:04d}.pt"),
            epoch + 1,
            total_loss,
            optimiser,
            train_param,
...
model.save(
    savefolder.joinpath("LPD_final_iter.pt"),
