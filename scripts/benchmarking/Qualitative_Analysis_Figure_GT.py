...
def plot_imgs(file_list,  savename, width=None, vmin=None, vmax=None):
    if width is None:
...
savepath = "/export/scratch3/mbk/LION/slice_182_recons/"
...
for algo in algo_names:
    for exp in experiment_names:
        model_name = str(savepath+algo+"_"+exp+".npy")
        model_names.append(model_name)

file_list = ["/export/scratch3/mbk/LION/slice_182_recons/GT_FullData.npy"]
...
plot_imgs(
    ...
    vmax=0.008,
    width=15,
    savename = "Qualitative_Analysis_Figure_GT.pdf"
)
...
for ax in axes[:len(file_list)]:
    add_zoom_bubble(ax.images[0], roi=(.6, .6), zoom=2,linewidth=0.5,
    inset_center=(0.2, 0.2), inset_radius=.2) #roi=(.4, .3), zoom=2, inset_radius=.2

plt.tight_layout()
plt.savefig("/export/scratch3/mbk/LION/slice_182_recons/Qualitative_Analysis_Figure_GT.pdf", dpi=300)
