...
def extract_recon(experiment, model, idx, savename):
...
model_names = ["FBPDnCNN_FullData_min_val.pt",
"FBPDnCNN_Limited120_min_val.pt", "FBPDnCNN_Limited90_min_val.pt", "FBPDnCNN_Limited60_min_val.pt",
"FBPDnCNN_Sparse360_min_val.pt", "FBPDnCNN_Sparse120_min_val.pt", "FBPDnCNN_Sparse60_min_val.pt",
"FBPDnCNN_LowDose_min_val.pt", "FBPDnCNN_BeamHardening_min_val.pt"]
...
for model_idx, model_name in enumerate(tqdm(model_names)):
    ...
    model, options, data = FBPDnCNN.load(savefolder.joinpath(model_name))
    model.to(device)
    extract_recon(experiments[model_idx], model, idx, savename)
