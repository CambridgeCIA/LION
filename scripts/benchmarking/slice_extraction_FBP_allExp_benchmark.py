...
def extract_recon(experiment, idx, savename):
    ...
    with torch.no_grad():
        for index, (data, target) in enumerate(tqdm(testing_dataloader)):
            if index == idx:
                recon = fdk(op,data[0].to(device)).detach().cpu().numpy().squeeze()
...
model_names = ["FBP_FullData_min_val.pt",
"FBP_Limited120_min_val.pt", "FBP_Limited90_min_val.pt", "FBP_Limited60_min_val.pt",
"FBP_Sparse360_min_val.pt", "FBP_Sparse120_min_val.pt", "FBP_Sparse60_min_val.pt",
"FBP_LowDose_min_val.pt", "FBP_BeamHardening_min_val.pt"]
