...
def extract_recon(experiment, idx, savename):
    ...
    with torch.no_grad():
        for index, (data, target) in enumerate(tqdm(testing_dataloader)):
            if index == idx:
                recon = nag_ls(op,data[0].to(device),num_iterations=100).detach().cpu().numpy().squeeze()
...
model_names = ["NAG_FullData_min_val.pt",
"NAG_Limited120_min_val.pt", "NAG_Limited90_min_val.pt", "NAG_Limited60_min_val.pt",
"NAG_Sparse360_min_val.pt", "NAG_Sparse120_min_val.pt", "NAG_Sparse60_min_val.pt",
"NAG_LowDose_min_val.pt", "NAG_BeamHardening_min_val.pt"]
...
for model_idx, model_name in enumerate(tqdm(model_names)):
    ...
    #model.to(device)
    extract_recon(experiments[model_idx],idx, savename)
