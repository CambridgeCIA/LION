...
def run_demo(dataset: torch.utils.data.Dataset, subtract_from_J: int = 1, delta_divided_by: int = 4):
    ...
    # print(f"Sampling rate:         {delta * 100:.2f}%")
    # print(f"In-order measurements: {1 / (1 << (subtract_from_J * 2)) * 100:.2f}%")
    ...
    def denoiser_fn_admm(x: torch.Tensor) -> torch.Tensor:
        with torch.no_grad():
            ...
            denoised = torch.mean(denoised, dim=1).squeeze(0)  # average the channels to get grayscale
        return denoised
