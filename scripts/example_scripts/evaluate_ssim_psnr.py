...
def plot_box(tensors, labels, dose, geometry, title, save):
    ...
    plt.ylabel(title)
    if save:
        plt.savefig(f"/home/cr661/rds/hpc-work/store/LION/trained_models/{DOSE}_dose/results/results_cLPDIN_LPD/{title}_{dose}_{geometry}.png")
    plt.show()
# %%
...
for i in out:
    ...
    print(f"Mean: {mean:.4f}, Std: {std:.4f}, 25th percentile: {percentile_25}, 50th percentile: {percentile_50}, 75th percentile: {percentile_75}")
...
# %%
# Plot SSIM
plot_box([out[0], out[1], out[2]], ['cLPD', 'LPD', 'FDK'], geometry=GEOMETRY, dose=DOSE, title="SSIM", save=True)
# %%
# Plot PSNR
plot_box([out[3], out[4], out[5]], ['cLPD', 'LPD', 'FDK'], geometry=GEOMETRY, dose=DOSE, title="PSNR", save=True)
# %%
