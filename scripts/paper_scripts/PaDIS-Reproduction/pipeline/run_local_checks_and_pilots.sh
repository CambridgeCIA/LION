...
. "$SCRIPT_DIR/../platforms/slurm/padis_a100_common.sh"
...
PADIS_LOCAL_VALIDATION_MAX_PATCHES="${PADIS_LOCAL_VALIDATION_MAX_PATCHES:-1000}"
PADIS_LOCAL_CHECKPOINT_INTERVAL_PATCHES="${PADIS_LOCAL_CHECKPOINT_INTERVAL_PATCHES:-512}"
PADIS_LOCAL_MAX_PERIODIC_CHECKPOINTS="${PADIS_LOCAL_MAX_PERIODIC_CHECKPOINTS:-5}"
PADIS_LOCAL_LOG_INTERVAL_PATCHES="${PADIS_LOCAL_LOG_INTERVAL_PATCHES:-128}"
PADIS_LOCAL_SEED="${PADIS_LOCAL_SEED:-33}"
...
export MPLCONFIGDIR WANDB_DIR PYTHONUNBUFFERED=1 OMP_NUM_THREADS=1 PYTHONHASHSEED="$PADIS_LOCAL_SEED"
...
if [ "$PADIS_LOCAL_SKIP_CHECKS" != "1" ]; then
        run_logged_with_budget py_compile \
                ...
                scripts/paper_scripts/PaDIS-Reproduction/training/PaDIS_LIDC_256.py \
                scripts/paper_scripts/PaDIS-Reproduction/training/PaDIS_LIDC_512.py \
                scripts/paper_scripts/PaDIS-Reproduction/reconstruction/PaDIS_LIDC_generation.py \
                scripts/paper_scripts/PaDIS-Reproduction/reconstruction/PaDIS_LIDC_reconstruction.py \
                scripts/paper_scripts/PaDIS-Reproduction/core/PaDIS_experiments.py
...
for task in "${selected_tasks[@]}"; do
        ...
        if [ "$task_engine" = "lidc256" ]; then
                cmd=(
                        ...
                        --validation-max-patches "$PADIS_LOCAL_VALIDATION_MAX_PATCHES"
                        --checkpoint-interval-patches "$PADIS_LOCAL_CHECKPOINT_INTERVAL_PATCHES"
                        --max-periodic-checkpoints "$PADIS_LOCAL_MAX_PERIODIC_CHECKPOINTS"
                        --log-interval-patches "$PADIS_LOCAL_LOG_INTERVAL_PATCHES"
                        --seed "$PADIS_LOCAL_SEED"
