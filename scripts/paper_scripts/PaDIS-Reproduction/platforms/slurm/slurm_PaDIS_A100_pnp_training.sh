...
PADIS_PNP_MAX_PERIODIC_CHECKPOINTS="${PADIS_PNP_MAX_PERIODIC_CHECKPOINTS:-5}"
...
PADIS_PNP_WANDB_NAME="${PADIS_PNP_WANDB_NAME:-${PADIS_WANDB_NAME_PREFIX:-PaDIS_A100_${PADIS_RUN_STAMP:-${SLURM_JOB_ID:-manual}}}_${PADIS_PNP_RUN_NAME}}"
export LION_ROOT PADIS_RUN_ROOT PADIS_TRAIN_ROOT MPLCONFIGDIR WANDB_DIR PYTHONUNBUFFERED=1 OMP_NUM_THREADS=1
...
if [ "$PADIS_NO_WANDB" = "1" ]; then
        ...
        CMD+=(
                ...
                --wandb-name "$PADIS_PNP_WANDB_NAME"
                --wandb-mode "$PADIS_WANDB_MODE"
        )
        if [ -n "$PADIS_WANDB_ENTITY" ]; then
                CMD+=(--wandb-entity "$PADIS_WANDB_ENTITY")
        fi
fi
