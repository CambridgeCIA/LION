#!/usr/bin/env bash
set -euo pipefail

echo "This pre-fixed-validation helper is obsolete." >&2
echo "Use scripts/paper_scripts/PaDIS-Reproduction/tuning/run_fixedval_reconstruction_tuning.sh instead." >&2
echo "Refusing to write contaminated/non-fixedval run names or summarize --run-names all." >&2
exit 1

SOCKET="/home/thomas/DiS/Project/Data/experiments/PaDIS/hparam_tuning/padis_tune.tmux"
PROJECT_ROOT="/home/thomas/DiS/Project/LION"

while tmux -S "$SOCKET" has-session -t lion_physics_solver_pc_followup 2>/dev/null; do
        sleep 300
done

cd "$PROJECT_ROOT"

conda run --no-capture-output -n lion-dev python scripts/paper_scripts/PaDIS-Reproduction/tuning/PaDIS_tune_reconstruction_hyperparameters.py \
        --candidate-set consensus_24h_no_defaults \
        --run-name paper_public_consensus_validation_20260705 \
        --methods padis_dps,whole_image_diffusion,langevin,predictor_corrector,ve_ddnm,patch_average,patch_stitch \
        --only-implementations paper,public_repo \
        --experiments ct_20,ct_8 \
        --max-samples 1 \
        --start-index 4 \
        --device cuda \
        --prog-bar

conda run --no-capture-output -n lion-dev python scripts/paper_scripts/PaDIS-Reproduction/tuning/PaDIS_summarize_hparam_tuning.py \
        --run-names all \
        --expected-experiments ct_20,ct_8 \
        --top-k 8 \
        --output-csv /home/thomas/DiS/Project/Data/experiments/PaDIS/hparam_tuning/runs/hparam_recommendations_with_public_deltas.csv
