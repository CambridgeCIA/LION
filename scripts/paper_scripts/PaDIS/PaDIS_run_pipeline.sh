...
Examples:
  ...
  PADIS_RUN_STAMP=paper-final bash scripts/paper_scripts/PaDIS/PaDIS_run_pipeline.sh --backend slurm
EOF
}
...
case "$BACKEND" in
        gcp)
                runner="$SCRIPT_DIR/gcp/run_PaDIS_GCP_spot_training.sh"
                ...
                ;;
        slurm)
                runner="$SCRIPT_DIR/slurm/submit_PaDIS_A100_pipeline.sh"
