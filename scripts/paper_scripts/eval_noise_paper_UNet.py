...
def plot_sinos(ExpNoise_testing_data, ArtNoise_testing_data, idx, model_1_name, model_1, model_2_name, model_2, qual, savename):
    ...
    plt.title("ExpDnsd_M1, PSNR: {:.2f}".format(Exp_UNet_ExpNoise_psnr[idx]))
    plt.axis("off")
    plt.clim(0, max_val)
    ...
    plt.title("ExpDnsd_M2, PSNR: {:.2f}".format(Exp_UNet_ArtNoise_psnr[idx]))
    plt.axis("off")
    plt.clim(0, max_val)
    ...
    plt.title("ArtDnsd_M1, PSNR: {:.2f}".format(Art_UNet_ExpNoise_psnr[idx]))
    plt.axis("off")
    plt.clim(0, max_val)
    ...
    plt.title("ArtDnsd_M2, PSNR: {:.2f}".format(Art_UNet_ArtNoise_psnr[idx]))
    plt.axis("off")
    plt.clim(0, max_val)
...
def plot_recons(ExpNoise_testing_data, ArtNoise_testing_data, idx, model_1_name, model_1, model_2_name, model_2, qual, savename):
    ...
    plt.title(f"ExpDnsd_M1, PSNR: {Exp_UNet_ExpNoise_psnr[idx]:.2f}")
    plt.clim(0, max_val)
    plt.axis("off")

    plt.subplot(336)
    plt.imshow(Art_clnd_recon_m1.detach().cpu().numpy().squeeze(), cmap="gray")
    plt.title(f"ArtDnsd_M1, PSNR: {Art_UNet_ExpNoise_psnr[idx]:.2f}")
    plt.clim(0, max_val)
    plt.axis("off")

    plt.subplot(338)
    plt.imshow(Exp_clnd_recon_m2.detach().cpu().numpy().squeeze(), cmap="gray")
    plt.title(f"ExpDnsd_M2, PSNR: {Exp_UNet_ArtNoise_psnr[idx]:.2f}")
    plt.clim(0, max_val)
    plt.axis("off")

    plt.subplot(339)
    plt.imshow(Art_clnd_recon_m2.detach().cpu().numpy().squeeze(), cmap="gray")
    plt.title(f"ArtDnsd_M2, PSNR: {Art_UNet_ArtNoise_psnr[idx]:.2f}")
    plt.clim(0, max_val)
    plt.axis("off")
...
model_1_name = "UNet_ExpNoise_min_val.pt"
model_2_name = "UNet_ArtNoise_min_val.pt"
...
model_1, options, data = UNet.load(savefolder.joinpath(model_1_name))
model_1.to("cpu")
model_1.eval()

model_2, options, data = UNet.load(savefolder.joinpath(model_2_name))
model_2.to("cpu")
model_2.eval()
...
if eval_domain == "sino":
    ...
    with torch.no_grad():
        for index, (sino, target) in enumerate(tqdm(ExpNoise_testing_dataloader)):
            ...
            Exp_UNet_ExpNoise_ssim[index] = my_ssim(target, output_m1)
            Exp_UNet_ExpNoise_psnr[index] = my_psnr(target, output_m1)

    with torch.no_grad():
        for index, (sino, target) in enumerate(tqdm(ArtNoise_testing_dataloader)):
            output_m1 = model_1(sino.to(device))
            Art_UNet_ExpNoise_ssim[index] = my_ssim(target, output_m1)
            Art_UNet_ExpNoise_psnr[index] = my_psnr(target, output_m1)
    ...
    with torch.no_grad():
        for index, (sino, target) in enumerate(tqdm(ExpNoise_testing_dataloader)):
            ...
            Exp_UNet_ArtNoise_ssim[index] = my_ssim(target, output_m2)
            Exp_UNet_ArtNoise_psnr[index] = my_psnr(target, output_m2)

    with torch.no_grad():
        for index, (sino, target) in enumerate(tqdm(ArtNoise_testing_dataloader)):
            output_m2 = model_2(sino.to(device))
            Art_UNet_ArtNoise_ssim[index] = my_ssim(target, output_m2)
            Art_UNet_ArtNoise_psnr[index] = my_psnr(target, output_m2)
    
    model_2.to("cpu")

    print(f"Sino domain results")
    print(f"UNet_ExpNoise Experimental SSIM: {Exp_UNet_ExpNoise_ssim.mean()} +- {Exp_UNet_ExpNoise_ssim.std()}")
    print(f"UNet_ExpNoise Experimental PSNR: {Exp_UNet_ExpNoise_psnr.mean()} +- {Exp_UNet_ExpNoise_psnr.std()}")
    print(f"UNet_ExpNoise Artificial SSIM: {Art_UNet_ExpNoise_ssim.mean()} +- {Art_UNet_ExpNoise_ssim.std()}")
    print(f"UNet_ExpNoise Artificial PSNR: {Art_UNet_ExpNoise_psnr.mean()} +- {Art_UNet_ExpNoise_psnr.std()}")

    print(f"UNet_ArtNoise Experimental SSIM: {Exp_UNet_ArtNoise_ssim.mean()} +- {Exp_UNet_ArtNoise_ssim.std()}")
    print(f"UNet_ArtNoise Experimental PSNR: {Exp_UNet_ArtNoise_psnr.mean()} +- {Exp_UNet_ArtNoise_psnr.std()}")
    print(f"UNet_ArtNoise Artificial SSIM: {Art_UNet_ArtNoise_ssim.mean()} +- {Art_UNet_ArtNoise_ssim.std()}")
    print(f"UNet_ArtNoise Artificial PSNR: {Art_UNet_ArtNoise_psnr.mean()} +- {Art_UNet_ArtNoise_psnr.std()}")

    # TEST 2: Display examples
    min_idx = np.argmin(Exp_UNet_ExpNoise_psnr)
    max_idx = np.argmax(Exp_UNet_ExpNoise_psnr)

    # find the closest to the mean
    mean_idx = arg_find_nearest(Exp_UNet_ExpNoise_psnr, Exp_UNet_ExpNoise_psnr.mean())
    ...
    # BEST PSNR
    plot_sinos(ExpNoise_testing_data, ArtNoise_testing_data, max_idx, model_1_name, model_1, model_2_name, model_2, "Best", "UNet_sinos_eval_best_psnr.png")

    # WORST PSNR
    plot_sinos(ExpNoise_testing_data, ArtNoise_testing_data, min_idx, model_1_name, model_1, model_2_name, model_2, "Worst", "UNet_sinos_eval_worst_psnr.png")

    # MEAN PSNR
    plot_sinos(ExpNoise_testing_data, ArtNoise_testing_data, mean_idx, model_1_name, model_1, model_2_name, model_2, "Mean", "UNet_sinos_eval_mean_psnr.png")
...
if eval_domain == "recon":
    ...
    with torch.no_grad():
        for index, (sino, target) in enumerate(tqdm(ExpNoise_testing_dataloader)):
            ...
            Exp_UNet_ExpNoise_ssim[index] = my_ssim(target, sino_recon_m1)
            Exp_UNet_ExpNoise_psnr[index] = my_psnr(target, sino_recon_m1)
    ...
    with torch.no_grad():
        for index, (sino, target) in enumerate(tqdm(ArtNoise_testing_dataloader)):
            output_m1 = model_1(sino.to(device))
            sino_recon_m1 = fdk(op, output_m1[0].to(device))
            Art_UNet_ExpNoise_ssim[index] = my_ssim(target, sino_recon_m1)
            Art_UNet_ExpNoise_psnr[index] = my_psnr(target, sino_recon_m1)
    ...
    with torch.no_grad():
        for index, (sino, target) in enumerate(tqdm(ExpNoise_testing_dataloader)):
            ...
            Exp_UNet_ArtNoise_ssim[index] = my_ssim(target, sino_recon_m2)
            Exp_UNet_ArtNoise_psnr[index] = my_psnr(target, sino_recon_m2)

    with torch.no_grad():
        for index, (sino, target) in enumerate(tqdm(ArtNoise_testing_dataloader)):
            output_m2 = model_2(sino.to(device))
            sino_recon_m2 = fdk(op, output_m2[0].to(device))
            Art_UNet_ArtNoise_ssim[index] = my_ssim(target, sino_recon_m2)
            Art_UNet_ArtNoise_psnr[index] = my_psnr(target, sino_recon_m2)
    ...
    # BEST PSNR
    plot_recons(ExpNoise_testing_data, ArtNoise_testing_data, max_idx, model_1_name, model_1, model_2_name, model_2, "Best", "UNet_recons_eval_best_psnr.png")

    # WORST PSNR
    plot_recons(ExpNoise_testing_data, ArtNoise_testing_data, min_idx, model_1_name, model_1, model_2_name, model_2, "Worst", "UNet_recons_eval_worst_psnr.png")

    # MEAN PSNR
    plot_recons(ExpNoise_testing_data, ArtNoise_testing_data, mean_idx, model_1_name, model_1, model_2_name, model_2, "Mean", "UNet_recons_eval_mean_psnr.png")