...
def plot_recons(ExpNoise_testing_data, ArtNoise_testing_data, idx, model_1_name, model_1, model_2_name, model_2, qual, savename):
    ...
    plt.title(f"ExpDnsd_M1, PSNR: {Exp_FBPMSDNet_ExpNoise_psnr[idx]:.2f}")
    plt.clim(0, max_val)
    plt.axis("off")

    plt.subplot(336)
    plt.imshow(Art_clnd_recon_m1.detach().cpu().numpy().squeeze(), cmap="gray")
    plt.title(f"ArtDnsd_M1, PSNR: {Art_FBPMSDNet_ExpNoise_psnr[idx]:.2f}")
    plt.clim(0, max_val)
    plt.axis("off")

    plt.subplot(338)
    plt.imshow(Exp_clnd_recon_m2.detach().cpu().numpy().squeeze(), cmap="gray")
    plt.title(f"ExpDnsd_M2, PSNR: {Exp_FBPMSDNet_ArtNoise_psnr[idx]:.2f}")
    plt.clim(0, max_val)
    plt.axis("off")

    plt.subplot(339)
    plt.imshow(Art_clnd_recon_m2.detach().cpu().numpy().squeeze(), cmap="gray")
    plt.title(f"ArtDnsd_M2, PSNR: {Art_FBPMSDNet_ArtNoise_psnr[idx]:.2f}")
    plt.clim(0, max_val)
    plt.axis("off")
...
#model_names = ["FBPMSDNet_ExpNoise.pt", "FBPMSDNet_ArtNoise.pt", "FBPMSDNet_ExpNoise.pt", "FBPMSDNet_ArtNoise.pt"]

model_1_name = "FBPMSDNet_ExpNoise_min_val.pt"
model_2_name = "FBPMSDNet_ArtNoise_min_val.pt"
...
model_1, options, data = FBPMS_D.load(savefolder.joinpath(model_1_name))
model_1.to("cpu")
model_1.eval()

model_2, options, data = FBPMS_D.load(savefolder.joinpath(model_2_name))
model_2.to("cpu")
model_2.eval()
...
if eval_domain == "recon":
    ...
    with torch.no_grad():
        for index, (sino, target) in enumerate(tqdm(ExpNoise_testing_dataloader)):
            ...
            Exp_FBPMSDNet_ExpNoise_ssim[index] = my_ssim(target, output_m1)
            Exp_FBPMSDNet_ExpNoise_psnr[index] = my_psnr(target, output_m1)
    ...
    with torch.no_grad():
        for index, (sino, target) in enumerate(tqdm(ArtNoise_testing_dataloader)):
            output_m1 = model_1(sino.to(device))
            #sino_recon_m1 = fdk(op, output_m1[0].to(device))
            Art_FBPMSDNet_ExpNoise_ssim[index] = my_ssim(target, output_m1)
            Art_FBPMSDNet_ExpNoise_psnr[index] = my_psnr(target, output_m1)
    ...
    with torch.no_grad():
        for index, (sino, target) in enumerate(tqdm(ExpNoise_testing_dataloader)):
            ...
            Exp_FBPMSDNet_ArtNoise_ssim[index] = my_ssim(target, output_m2)
            Exp_FBPMSDNet_ArtNoise_psnr[index] = my_psnr(target, output_m2)

    with torch.no_grad():
        for index, (sino, target) in enumerate(tqdm(ArtNoise_testing_dataloader)):
            output_m2 = model_2(sino.to(device))
            #sino_recon_m2 = fdk(op, output_m2[0].to(device))
            Art_FBPMSDNet_ArtNoise_ssim[index] = my_ssim(target, output_m2)
            Art_FBPMSDNet_ArtNoise_psnr[index] = my_psnr(target, output_m2)

    model_2.to("cpu")

    print(f"Recon domain results")
    print(f"FBPMSDNet_ExpNoise Experimental SSIM: {Exp_FBPMSDNet_ExpNoise_ssim.mean()} +- {Exp_FBPMSDNet_ExpNoise_ssim.std()}")
    print(f"FBPMSDNet_ExpNoise Experimental PSNR: {Exp_FBPMSDNet_ExpNoise_psnr.mean()} +- {Exp_FBPMSDNet_ExpNoise_psnr.std()}")
    print(f"FBPMSDNet_ExpNoise Artificial SSIM: {Art_FBPMSDNet_ExpNoise_ssim.mean()} +- {Art_FBPMSDNet_ExpNoise_ssim.std()}")
    print(f"FBPMSDNet_ExpNoise Artificial PSNR: {Art_FBPMSDNet_ExpNoise_psnr.mean()} +- {Art_FBPMSDNet_ExpNoise_psnr.std()}")

    print(f"FBPMSDNet_ArtNoise Experimental SSIM: {Exp_FBPMSDNet_ArtNoise_ssim.mean()} +- {Exp_FBPMSDNet_ArtNoise_ssim.std()}")
    print(f"FBPMSDNet_ArtNoise Experimental PSNR: {Exp_FBPMSDNet_ArtNoise_psnr.mean()} +- {Exp_FBPMSDNet_ArtNoise_psnr.std()}")
    print(f"FBPMSDNet_ArtNoise Artificial SSIM: {Art_FBPMSDNet_ArtNoise_ssim.mean()} +- {Art_FBPMSDNet_ArtNoise_ssim.std()}")
    print(f"FBPMSDNet_ArtNoise Artificial PSNR: {Art_FBPMSDNet_ArtNoise_psnr.mean()} +- {Art_FBPMSDNet_ArtNoise_psnr.std()}")


    # TEST 2: Display examples
    min_idx = np.argmin(Exp_FBPMSDNet_ExpNoise_psnr)
    max_idx = np.argmax(Exp_FBPMSDNet_ExpNoise_psnr)

    # find the closest to the mean
    mean_idx = arg_find_nearest(Exp_FBPMSDNet_ExpNoise_psnr, Exp_FBPMSDNet_ExpNoise_psnr.mean())
    ...
    # BEST PSNR
    plot_recons(ExpNoise_testing_data, ArtNoise_testing_data, max_idx, model_1_name, model_1, model_2_name, model_2, "Best", "FBPMSDNet_recons_eval_best_psnr.png")

    # WORST PSNR
    plot_recons(ExpNoise_testing_data, ArtNoise_testing_data, min_idx, model_1_name, model_1, model_2_name, model_2, "Worst", "FBPMSDNet_recons_eval_worst_psnr.png")

    # MEAN PSNR
    plot_recons(ExpNoise_testing_data, ArtNoise_testing_data, mean_idx, model_1_name, model_1, model_2_name, model_2, "Mean", "FBPMSDNet_recons_eval_mean_psnr.png")
