...
if __name__ == "__main__":
    ...
    with (measurement_file_dir / measurement_file_name).open("r", encoding="utf-8", errors="ignore") as f:
        ...
        # There may be multiple measurements for each pattern index.
        # We need to average the measurements for each pattern index.
        ...
        hadamard_measurement_vector = (out[:, 1] * sign_vector).reshape(-1, 2).sum(axis=1)
        ...
        np.save(measurement_file_dir / f"{name}_hadamard_measurement_vector.npy", hadamard_measurement_vector)
    ...
    reconstructed_image_np = reconstructed_image_torch.cpu().numpy().reshape((2**J, 2**J))
    ...
    np.save(measurement_file_dir / f"{name}_reconstructed_image.npy", reconstructed_image_np)
    ...
    spyrit_reconstructed_image_torch = ifwht(hadamard_measurement_vector_torch, order=True)
    spyrit_reconstructed_image_np = spyrit_reconstructed_image_torch.cpu().numpy().reshape((2**J, 2**J))
    ...
    np.save(measurement_file_dir / f"{name}_spyrit_reconstructed_image.npy", spyrit_reconstructed_image_np)
    ...
    plt.tight_layout()
    plt.savefig(measurement_file_dir / f"{name}_spyrit_reconstructed_image.png", dpi=300)
    plt.close()
